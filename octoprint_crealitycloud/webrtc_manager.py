from aiortc import RTCIceCandidate, RTCSessionDescription, RTCConfiguration, RTCIceServer, RTCRtpSender, RTCPeerConnection
...
class WebrtcManager():
    ...
    async def signaling_message_handler(self, ws, message):
        ...
        if(action == "ice_msg"):
            ...
            if(type == "offer"):
                ...
                self._logger.debug(f"offer received from {peerId[:12]}")
                self._logger.debug(f"iceServers:{iceServer}")
    ...
    async def add_peer(self, peer_id, peer_type, polite, media):
        if(peer_id in self.peers):
            if(self.verbose):
                self._logger.debug("A peer connection with id", peer_id, "already exists")
        else:
            iceServers = []
            iceServer = {"urls": self.urls, "username": self.username, "credential":self.credential}
            self._logger.debug(f"iceServer:{iceServer}")
    ...
    def update_streams_logic(self, peer):
        ...
        if self.options.get('enableRemoteStream'):
            ...
            self._logger.debug(peer['remoteVideoStream'])
    ...
    async def update_negotiation_logic(self, peer):
        ...
        try:
            if(not peer["polite"]):
                ...
                if(self.verbose):
                    self._logger.debug(f"Sending offer to {peer['peerId']}")
                data = {
                        "action": "ice_msg",
                        "sdpmessage": 
                                {
            ...
            self._logger.warning('Something related to update_negotiation_logic went wrong')
        ...
        try:
            ...
            if(offerCollision):
                ...
                if(peer.get('remoteVideoStream')):
                    self._logger.debug("### STARTING REMOTE STREAM this is the remote stream",peer.get('remoteVideoStream'))
                    await peer.get('remoteVideoStream').start()
            
            if(description["type"] == 'offer'):
                await peerConnection.setLocalDescription(await peerConnection.createAnswer())
                if(self.verbose):
                    self._logger.debug(f"sending answer to {peer['peerId'][:12]}")
                data = {
                        "action": "ice_msg",
                        "sdpmessage": 
                                {
            ...
            self._logger.warning("Something related to update_session_description went wrong")
    ...
    async def update_ice_candidate(self, peer, candidate):
        ...
        if data == "":
            self._logger.debug("candidate is void")
        else:
            ...
            try:
                if(candidate != None):  # address == ip
                    self._logger.debug(f"adding ICE candidate: {protocol} {address}:{port} type={type}")
                ...
                if(not peer["ignoreOffer"]):
                    self._logger.warning(f"addIceCandidate failed: {e}")
    ...
    async def remove_peer(self, peerId):
        ...
        for peer_id in peer_ids:                
            if peerId == peer_id:
                ...
                if(self.verbose):
                    self._logger.debug(f"Connection with {peerId[:12]} has been removed")
    ...
    def track_states(self, peer):
        ...
        def iceconnectionstatechange():
            self._logger.debug(f"ICE state: {pc.iceConnectionState} for {disconnect_id[:12]}")
            timestamp = time.time()
            states = {
    ...
    def push_state(self, ws):     
        ...
        for peer_id in peer_ids:
            ...
            self._logger.debug(str(pushState))
