...
class Recorder(object):
    def __init__(self, path, stream_url=None):
        self.mjpg_stream_url = stream_url or "http://127.0.0.1/webcam/?action=stream"
        self.timer = None
        self.ffmpeg = None
        self._logger = logging.getLogger("octoprint.plugins.crealitycloudrecorder")
        self._limit_size = 500 # limit size 500MB
        self._printid = None
        self._recorder_file_path = f"{path}/creality_recorder"
        self._recorder_list_path = f"{path}/creality_recorder/vlist.json"
    ...
    def get_new_recorder_hour_dir(self):
        ...
        #                                                                                                         time.localtime())
        path = f"{self._recorder_file_path}/{time.strftime('%Y-%m-%d', time.localtime())}/{self._printid}"
    ...
    def get_hour_dir_list(self, date):
        path = f"{self._recorder_file_path}/{date}"
        return os.listdir(path)

    def get_min_dir_list(self, date, hour):
        path = f"{self._recorder_file_path}/{date}/{hour}"
        return os.listdir(path)
        ...
        if os.access(self._recorder_list_path,os.F_OK):
            ...
            with open(self._recorder_list_path,'w',encoding='utf-8') as dump_f:                                   
                ...
                for i, date_dict in enumerate(load_list):                       
                    for i in date_dict:       
                        date = i 
                        record_path = f"{self._recorder_file_path}/{date}"
    ...
    def start_play(self,path):
        ...
        FFmpeg play video
        """        
        outpath = f'rtsp://127.0.0.1:8554/{path}'
    ...
    def create_play_list(self,path):
        ...
        filepath = f"{self._recorder_file_path}/{datepath}/{printidpath}"
        listpath = f"{filepath}/playlist.txt"
    ...
    def concat_video(self):
        self._logger.debug('start concat video')
        filepath = self.get_new_recorder_hour_dir()
        listpath = f"{filepath}/playlist.txt"
        outputpath = f"{filepath}/output.mp4"
        # 返回path下所有文件构成的一个list列表
        if os.access(listpath,os.F_OK):    
            os.remove(listpath)     
        filelist=os.listdir(filepath)
                        ...
                        for i, print_dict in enumerate(printid_list):        
                            for i in print_dict:      
                                ...
                                if tss == tsp:            
                                    ...
                                    #tsx = (tsp -tss)/10
                                    #self.flag = 1
        filepath = f"{self._recorder_file_path}/{datepath}/{printidpath}/output.mp4"
        return filepath
        ...
        for root, dirs, files in os.walk(path):
            for name in files:
                if name.endswith("output.mp4"):
                    ...
                    print(f"Delete File: {os.path.join(root, name)}")
